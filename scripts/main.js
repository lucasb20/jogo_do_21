...
function update(){
    ...
    for(let i=0;i<player_hand.length;i++){
        draw_card(player_hand[i],pdeck_x+20*i,pdeck_y)
    }
...
window.addEventListener('keydown',(event) => {
    switch(event.key){
        case 'd':
            if(!state_hold) deal(deck,player_hand)
            break
        case 'h':
            hold()
            break
...
function hold(){
    ...
    ctx.font = '20px Arial'
    if(check_winner()==1){
        ctx.fillText('Player wins.',res_posx,res_posy)
    }
